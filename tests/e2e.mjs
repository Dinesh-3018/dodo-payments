...
await frame.getByRole("button", { name: "Increase quantity" }).click();
ok("two perks at qty 3 and total updates", (await frame.locator(".perk.is-unlocked").count()) === 2 && (await frame.locator(".pay").textContent()) === "Pay ₹507");
ok("away line names the next perk", (await frame.locator(".perks-line").textContent())?.includes("₹492 away from a free tote bag"));
await frame.getByRole("button", { name: "Decrease quantity" }).click();
await frame.getByRole("button", { name: "Decrease quantity" }).click();
ok("back to qty 1", (await frame.locator(".pay").textContent()) === "Pay ₹169" && (await frame.getByRole("button", { name: "Decrease quantity" }).isDisabled()));
...
await frame.getByRole("button", { name: /^Pay ₹169$/ }).click();
...
ok("host quantity honoured", (await frame.locator(".stepper-value").textContent()) === "2" && (await frame.locator(".pay").textContent()) === "Pay ₹338");
await page.keyboard.press("Escape");
await waitOverlay(false);
await page.click('[data-qty="-1"]');
...
await declFrame.getByRole("button", { name: /^Pay ₹338$/ }).click();
